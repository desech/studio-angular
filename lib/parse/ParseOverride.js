...
module.exports = {
  ...
  injectInner (node, ref, overrides, data) {
    ...
    node.innerHTML = ''
    node.setAttributeNS(null, 'desech-regex-override-inner', `d.${ref}Inner`)
  },
  ...
  setAttributeRegex (node, ref, name, regex) {
    ...
    regex[ref].attributes[name] = `[attr.${name}]="d.${index}"`
  },
  ...
  injectTag (node, ref, overrides, data, document) {
    ...
    this.addTemplate(ref, overrides.tag, data.defaults[ref].tag, data.templates)
    // node.setAttributeNS(null, 'desech-regex-override-is', `d.${ref}Tag`)
    // return Html.changeTag(node, 'component', document)
  },

  addTemplate (ref, overrides, current, templates) {
    const array = ExtendJS.unique([...overrides, current])
  ...
  injectComponent (node, ref, overrides, data, document) {
    ...
    this.addTemplate(ref, overrides.component, node.tagName, data.templates)
