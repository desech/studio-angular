...
module.exports = {
  getIndexHtml (html) {
    ...
    html = html.replace(/<body>[\s\S]*<\/body>/g, `<body>${os.EOL}  <app-root></app-root>` +
      `${os.EOL}</body>`)
    return html
  },
  ...
  getClassImport (folder, file) {
    ...
    return `import { ${cls}Component } from './${dir}/${name}.component';${os.EOL}`
  },

  getAppModuleClass (folder, file) {
    const cls = ParseCommon.getClassName(file.name)
    return `    ${cls}Component,${os.EOL}`
  },
  ...
  getRoutePath (folder, file) {
    ...
    const cls = ParseCommon.getClassName(file.name)
    return `  { path: '${route}', component: ${cls}Component },${os.EOL}`
  },
