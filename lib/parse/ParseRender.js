...
module.exports = {
  ...
  getRenderBlock (body, file, data, lib) {
    ...
    return this.prepareData(html, component, this._component.templates)
  },

  reset () {
    this._component = { regex: {}, defaults: {}, templates: {} }
  },

  prepareData (html, component, templates) {
    if (!ExtendJS.isEmpty(this._component.defaults)) {
      component.defaults = this._component.defaults
    }
    return { html, component, templates }
  },
  ...
  getComponentNode (document, node, folder) {
    ...
    const cmpInstance = ParseCommon.getSelector(data.file, folder)
  ...
  injectComponentHole (document) {
    for (const node of document.querySelectorAll('[data-ss-component-hole]')) {
      ...
      node.innerHTML = '<ng-content></ng-content>'
    }
  },
  ...
  processMainComponentData (file, document) {
    ...
    return { variants: mainData.variants }
  }
}
